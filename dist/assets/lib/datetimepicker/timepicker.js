...
(function($){
  ...
  function _createTimepicker($input){
    ...
    header_div.className = "time-table__header";
    select_div.className = "time-table__select";
    button_div.className = "time-table__button";
    hour_header.className = "time-table__hour";
    min_header.className = "time-table__min";
  ...
  function closeSelect( $input ){
    if(( (!!$('.hour-select').val() && !!$('.min-select').val()))  )
  ...
  function _bindTimepicker($picker, $input){
    ...
    $picker.find(".hour-select").on("change", function(e){
     
      const hour =  $('.hour-select').val()
      const min =  $('.min-select').val() ?? '00'
      ...
      //2022.08.21 추가 
      if(e.target.parentElement.parentElement?.parentElement?.className.includes('ui-input-block--time-small')){
         _removeTimepicker($input);
      }
    });

      //custom start
    $picker.find(".min-select").on("change", function(e){
      const hour =  $('.hour-select').val()
      const min =  $('.min-select').val() ?? '00'
 ...
 // /custrom end
  ...
  /* 2022.11.30 : add : timepicker 엔터 적용 */
  $(".ui-input-block--time input").on("keydown", function(e) {
    var $this = $(this);
    var keyCode = e.keyCode || e.which;
    var removeAfterEnter = function _removeTimepicker($input){
      var $picker = $input.next(".my-timepicker-div");
      var $pickerParent = $picker.parent();
      $picker.remove();
      $pickerParent.removeClass('is-active && is-focus');
    }
    if (keyCode == 13) {
      removeAfterEnter($this);
    } else {
        return;
    }
  });
